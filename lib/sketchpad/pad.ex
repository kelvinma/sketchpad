defmodule Sketchpad.Pad do
  ...
  def clear(pad_id) do
    pad_id |> find!() |> GenServer.call(:clear)
  end

  def render(pad_id) do
    pad_id |> find!() |> GenServer.call(:render)
  end
  ...
  def handle_info(:request_png, state) do
    # List all presences on a particular topic
    case SketchpadWeb.Presence.list(state.topic) do
      users when users != %{} ->
        priv_topic = find_random_user_topic(users, state.topic)
        Phoenix.PubSub.broadcast(Sketchpad.PubSub, priv_topic, :request_png)
      _ -> :noop
    end
    schedule_png_request()
    {:noreply, state}
